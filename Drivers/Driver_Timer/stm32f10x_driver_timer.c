...
volatile u16 loop600HzCnt,loop500Hzcnt,loop200HzCnt,loop100HzCnt,loop50HzCnt,loop20HzCnt,loop10HzCnt=0;
...
void TIMx_Configuration(void)
{
  ...
  TIM_PrescalerConfig(TIM3, prescalerValue, TIM_PSCReloadMode_Immediate); // 预分频,现在计时器频率为2MHz
  ...
  TIM_OC1Init(TIM2, &TIM_OCInitStructure);  									    // 初始化外设TIM2 OC1-->Motor1
  TIM_OC2Init(TIM2, &TIM_OCInitStructure);  									    // 初始化外设TIM2 OC2-->Motor2
  TIM_OC3Init(TIM2, &TIM_OCInitStructure);  									    // 初始化外设TIM2 OC3-->Motor3
  TIM_OC4Init(TIM2, &TIM_OCInitStructure);  									    // 初始化外设TIM2 OC4-->Motor4

  //-----其他配置-----//
  TIM_ARRPreloadConfig(TIM2, ENABLE);
  TIM_ARRPreloadConfig(TIM3, ENABLE);											    // 自动重载寄存器使能，下一个更新事件自动更新影子寄存器
  TIM_OC2PreloadConfig(TIM2, TIM_OCPreload_Enable);  					// 使能TIM2在CCR2上的预装载寄存器,在更新事件时，值才被写入到CCR
  TIM_ITConfig(TIM3, TIM_IT_Update, ENABLE);									// 打开更新事件中断
