...
const u8 TX_ADDRESS[TX_ADR_WIDTH] = {0x34,0xc3,0x10,0x10,0x00}; // 发送地址
//const u8 RX_ADDRESS[RX_ADR_WIDTH] = {0x34,0xc3,0x10,0x10,0x00}; // 接收地址
//u8 RX_ADDRESS[RX_ADR_WIDTH] = {0x34,0xc3,0x10,0x10,0x26}; // 接收地址
...
void Nrf_Irq(void)
{
    ...
    if(sta & (1 << RX_DR))//接收轮训标志位
    {
		...
		NRF24L01_Write_Reg(0x27, sta);//清除nrf的中断标志位
		NRF24L01_Write_Reg(FLUSH_RX, 0xFF); //解决只能接收到第一次数据的bug
		sta = 0;
    }
